...
int
operator>> (const pipette::fifo &pipe, std::integral auto &T)
{
  return pipe.read (&T, sizeof (T));
}

int
operator<< (const pipette::fifo &pipe, const std::integral auto &T)
{
...
template <typename T>
void
operator<< (const pipette::fifo &pipe, const Point3D<T> &pnt)
{
  pipe << pnt.x;
  pipe << pnt.y;
  pipe << pnt.z;
...
int
operator>> (const pipette::pipe_bi &pipe, std::integral auto &T)
{
  return pipe.read ((void *)&T, sizeof (T));
}

int
operator<< (const pipette::pipe_bi &pipe, const std::integral auto &T)
{
...
template <typename T>
void
operator<< (const pipette::pipe_bi &pipe, const Point3D<T> &pnt)
{
  pipe << pnt.x;
  pipe << pnt.y;
  pipe << pnt.z;
  ...
  while (true)
    {
      ...
      pfront << game.food;
      for (const auto &piece : game.snek)
        pfront << piece;
    }
}
